...
def joern_parse(file, outdir):
    ...
    os.environ['out'] = str(out) #the filename parsed must equal to the source filename
    os.system('sh joern-parse $file --language c --output $out')
    with open(record_txt, 'a+') as f:
        f.writelines(name+'\n')

def joern_export(bin, outdir, repr):
    record_txt = os.path.join(outdir,f"export_res_{repr}.txt")
    if not os.path.exists(record_txt):
        os.system("touch "+record_txt)
    with open(record_txt,'r') as f:
        rec_list = f.readlines()
    ...
    os.environ['out'] = str(f"{outdir}/pooling/{name}")
    
    if repr in ['all', 'ast', 'cfg', 'ddg', 'cdg', 'pdg', 'cpg14']:
        os.system('sh joern-export $bin'+ " --repr " + repr + ' --out $out') 
        try:
            pdg_list = os.listdir(f"{outdir}/pooling/{name}")
            for pdg in pdg_list:
                if pdg.startswith(f"1-{repr}"):
                    if not os.path.exists(f"{outdir}/{name}"): os.mkdir(f"{outdir}/{name}")
                    os.system(f"mv {outdir}/pooling/{name}/{pdg} {outdir}/{name}/{repr}.dot")
                    os.system("rm -rf $out")
                    break
            with open(record_txt, 'a+') as f:
                f.writelines(name+'\n')
...
def main():
    ...
    if input_path[0] == '.': input_path = os.getcwd() + input_path[1:]
    if input_path[-1] != '/': input_path += '/'

    if output_path[0] == '.': output_path = os.getcwd() + output_path[1:]
    ...
    if type == 'parse':
        ...
        os.makedirs(f"{output_path}/pooling", exist_ok=True)
        pool.map(partial(joern_export, outdir=output_path, repr=repr), bins)
        os.system(f"rm -rf {output_path}/pooling")

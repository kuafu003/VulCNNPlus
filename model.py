...
def get_MCM_score(labels, predictions):
    ...
    M_f1 = f1_array.mean()
    W_fpr = (fpr_array * sum_array).sum() / sum_array.sum()
    W_fnr = (fnr_array * sum_array).sum() / sum_array.sum()
    W_f1 = (f1_array * sum_array).sum() / sum_array.sum()
    return {
...
class TraditionalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        vectors_turple = ()
        for graph_vec in feature:
            vectors = numpy.zeros(shape=(feature_num, self.max_len, self.hidden_size))
            for j in range(feature_num):
                for i in range(min(len(graph_vec[j]), self.max_len)):
                    vectors[j][i] = graph_vec[j][i]
            vectors_turple += (vectors, )
        return {
            # 'label': id,
            'vector': vectors_turple,
...
class CNN_Classifier():
    def __init__(self, max_len=100, n_classes=2, epochs=100, batch_size = 32, learning_rate = 0.001, \
                    result_save_path = "/root/vulcnn/data/results", item_num = 0, hidden_size = 128):
    ...
    def fit(self):
        ...
        for i, data in progress_bar:
            ...
            vectors = [vec.to(self.device) for vec in data["vector"]]
    ...
    def train(self):
        ...
        val_table = PrettyTable(['typ', 'epo', 'loss', 'M_fpr', 'M_fnr', 'M_f1', 'W_fpr', 'W_fnr', 'W_f1', 'ACC'])
        ...
        for epoch in range(self.epochs):
            ...
            val_loss, val_score = self.eval(self.valid_loader)
            val_table.add_row(["val", str(epoch+1), format(val_loss, '.4f')] + [val_score[j] for j in val_score if j != "MCM"])
            print(val_table)
            print("\n")
            if self.test_loader:
                test_loss, test_score = self.eval(self.test_loader)
                test_table.add_row(["tes", str(epoch+1), format(test_loss, '.4f')] + [test_score[j] for j in test_score if j != "MCM"])
